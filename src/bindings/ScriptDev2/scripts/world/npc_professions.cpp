...
/*###
# npc_leather_respec (temporary leatherworking respecialisation NPC - naraian southfancy)
###*/

bool GossipHello_npc_leather_respec(Player* pPlayer, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());
    if (pCreature->isVendor())
    ...
    if (pPlayer->HasSkill(SKILL_LEATHERWORKING) && pPlayer->GetBaseSkillValue(SKILL_LEATHERWORKING)>=250 && pPlayer->getLevel() > 49)
        if (pPlayer->GetQuestRewardStatus(5141) ||  pPlayer->GetQuestRewardStatus(5143) || pPlayer->GetQuestRewardStatus(5144)         //alliance
            || pPlayer->GetQuestRewardStatus(5145) || pPlayer->GetQuestRewardStatus(5146) || pPlayer->GetQuestRewardStatus(5148))      //horde
            if (!pPlayer->HasSpell(S_DRAGON) && !pPlayer->HasSpell(S_ELEMENTAL) && !pPlayer->HasSpell(S_TRIBAL))
            {
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_TRIBAL,    GOSSIP_SENDER_LEARN,    GOSSIP_ACTION_INFO_DEF + 1);
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_ELEMENTAL, GOSSIP_SENDER_LEARN,    GOSSIP_ACTION_INFO_DEF + 2);
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_DRAGON,    GOSSIP_SENDER_LEARN,    GOSSIP_ACTION_INFO_DEF + 3);
            }
...
bool GossipSelect_npc_leather_respec(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_TRADE:
        ...
        case GOSSIP_ACTION_TRAIN:
            ...
            //Learn Tribal
        case GOSSIP_ACTION_INFO_DEF + 1:
            if (!pPlayer->HasSpell(S_TRIBAL) && pPlayer->GetMoney() >= DoLearnCost(pPlayer))
            {
            ...
            //Learn Elemental
        case GOSSIP_ACTION_INFO_DEF + 2:
            if (!pPlayer->HasSpell(S_ELEMENTAL) && pPlayer->GetMoney() >= DoLearnCost(pPlayer))
            {
            ...
            //Learn Dragon  
        case GOSSIP_ACTION_INFO_DEF + 3:
            if (!pPlayer->HasSpell(S_DRAGON) && pPlayer->GetMoney() >= DoLearnCost(pPlayer))
            {
...
void AddSC_npc_professions()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_leather_respec";
    newscript->pGossipHello =  &GossipHello_npc_leather_respec;
    newscript->pGossipSelect = &GossipSelect_npc_leather_respec;
    newscript->RegisterSelf();
}
