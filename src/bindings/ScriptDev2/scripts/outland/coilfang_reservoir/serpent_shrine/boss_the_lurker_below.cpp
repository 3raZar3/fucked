	...
	void UpdateAI(const uint32 diff)
	{
        ...
        if (Phase1)
        {
			...
			if (CoilfangFrenzyTimer < diff)
			{
				...
				if(target && !m_creature->IsWithinDistInMap(target, 3))
					if(target->IsInWater())
						SummonAdd(MOBID_COILFANG_FRENZY, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ());
			...
			{
				Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 0);
				if (target) m_creature->CastSpell(target, SPELL_GEYSER, true);
				GeyserTimer = 10000 + rand()%5000;
			}GeyserTimer -= diff;
